...
"""
    ...
    def extract_outline_structure(self, outline: str) -> Dict[str, Any]:
        ...
        prompt = f"""
あなたはSEO対策に精通したプロのライターです。以下の条件に基づいて、SEO最適化された高品質なブログ記事を作成してください。
...
2. 導入部分（読者の関心を引く）
3. 適切な見出し構造（H2, H3を使用）
4. 各セクションで実用的で詳細な情報を提供
5. 具体例や実践的な手順を豊富に含める
6. まとめ・結論部分
7. メタディスクリプション提案

## SEO対策要件
- メインキーワードを自然に含める（密度2-3%程度）
- 見出しにキーワードを適切に配置
- 読みやすい文章構成
- 実用的で価値のある詳細情報を提供
- 読者の検索意図に応える包括的な内容
...
## 記事の詳細度要件
- 各H2セクションは300-500文字程度で詳しく説明
- 具体例、手順、tips等を豊富に含める
- 読者が実際に行動できる実践的な情報を提供
- 専門的な内容も初心者にわかりやすく解説
...
"""
    ...
    def generate_article(self, prompt: str) -> Optional[str]:
        """記事の生成"""
        if not self.client:
            return None
